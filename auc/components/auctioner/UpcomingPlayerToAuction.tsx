...
const UpcomingPlayersToAuction = () => {
  interface Player {
    setNumber: number;
    setYear: string;
    firstName: string;
  ...
  useEffect(() => {
    const fetchUpcomingPlayers = async () => {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/Auctioneer/upcoming-players?page=${currentPage}&limit=${pageSize}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${searchTerm}`
      );
      ...
      setPageSize(10);
      console.log(players); // Ensure filteredPlayers is initialized
    };
...
export default UpcomingPlayersToAuction;
